...
static bool appendAnecdote(const String& mmdd, const String& text) {
  ...
  File f = SD.open(fileForMMDD(mmdd), "a");
  if (!f) return false;
  f.println(t);
  f.close();
  return true;
...
static bool deleteAnecdote(const String& mmdd, int idx) {
  ...
  v.erase(v.begin() + idx);
  File f = SD.open(fileForMMDD(mmdd), "w");
  if (!f) return false;
...
<option value="4">Elancee</option>
<option value="5">Grasse</option>
</select>
</div>
<div class="row">
...
static void handleSettingsSet() {
  ...
  if (font.length()) CFG.font = (FontMode)clampI(font.toInt(), 0, 5);
...
void setup() {
  ...
  if (!SD.begin(SD_CS)) Serial.println("SD mount FAIL");
